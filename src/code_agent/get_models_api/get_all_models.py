...
class GetModels:
    ...
    async def nvidia_structured(self) -> List[Dict[str, Any]]:
        nvidia_structured = await asyncio.to_thread(
            self.nvidia.get_models_nvidia_structured
        )
        list_nvidia_structured = [  # type: ignore #
            {"model": model, "provider": "nvidia"} for model in nvidia_structured
        ]
    ...
    async def nvidia_tools(self) -> List[Dict[str, str]]:
        ...
        list_nvidia_tools = [  # type: ignore
            {"model": model, "provider": "nvidia"} for model in nvidia_tools
        ]
    ...
    async def nvidia_coder(self) -> List[Dict[str, str]]:
        ...
        list_nvidia_coder = [  # type: ignore
            {"model": model, "provider": "nvidia"} for model in nvidia_coder
        ]
    ...
    async def nvidia_chat(self) -> List[Dict[str, str]]:
        ...
        list_nvidia_chat = [  # type: ignore
            {"model": model, "provider": "nvidia"} for model in nvidia_chat
        ]
    ...
    async def groq_structured(self) -> List[Dict[str, str]]:
        ...
        list_groq_structured = [
            {"model": model, "provider": "groq"} for model in groq_structured
        ]
        return list_groq_structured

    async def cerebras_structured(self) -> List[Dict[str, str]]:
        cerebras_structured = await asyncio.to_thread(
            self.cerebras.get_models_cerebras_structured
        )
        list_cerebras_structured = [  # type: ignore
            {"model": model, "provider": "cerebras"} for model in cerebras_structured
        ]
    ...
    async def all_model_nvidia(self) -> List[Dict[str, str]]:
        ...
        list_all_model_nvidia = [  # type: ignore
            {"model": model, "provider": "nvidia"} for model in all_model_nvidia
        ]
    ...
    async def all_model_groq(self) -> List[Dict[str, str]]:
        ...
        list_all_model_groq = [
            {"model": model, "provider": "groq"} for model in all_model_groq
        ]
    ...
    async def all_models_cerebras(self) -> List[Dict[str, str]]:
        ...
        list_all_model_cerebras = [
            {"model": model, "provider": "cerebras"} for model in all_model_cerebras
        ]
        return list_all_model_cerebras

    async def get_all_models_async(
        self,
    ) -> Dict[str, List[Dict[str, str]] | BaseException]:
        ...
        return {
            "nvidia_structured": (
                results[0] if not isinstance(results[0], Exception) else results[0]
            ),
            "nvidia_tools": (
                results[1] if not isinstance(results[1], Exception) else results[1]
            ),
            "nvidia_coder": (
                results[2] if not isinstance(results[2], Exception) else results[2]
            ),
            "nvidia_chat": (
                results[3] if not isinstance(results[3], Exception) else results[3]
            ),
            "groq_structured": (
                results[4] if not isinstance(results[4], Exception) else results[4]
            ),
            "cerebras_structured": (
                results[5] if not isinstance(results[5], Exception) else results[5]
            ),
            "all_nvidia": (
                results[6] if not isinstance(results[6], Exception) else results[6]
            ),
            "all_groq": (
                results[7] if not isinstance(results[7], Exception) else results[7]
            ),
            "all_cerebras": (
                results[8] if not isinstance(results[8], Exception) else results[8]
            ),
        }

    async def salvar_modelos(self, path: str) -> None:
        all_models = await self.get_all_models_async()
        with open(f"{path}/all_models.json", "w", encoding="utf-8") as f:
