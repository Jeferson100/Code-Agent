...
# Usuário
create_box(ax, 6, 11, 2.5, 1, 'Usuario\n(Input)', color_user)

# Agente ReAct (LLM) - Central
create_box(ax, 6, 8.5, 3.5, 1.5, 'CodeAgentReact\n(LLM + ReAct Pattern)\nReasoning + Acting', color_agent, fontsize=11)
...
create_box(ax, 10.5, 4.5, 2, 1, 'write_code\nGerar Codigo\n(usa grafo interno)', color_tool, fontsize=9)
...
create_arrow(ax, 6, 10.5, 6, 9.5, '1. Mensagem', color_flow, offset=0.4)

# Agente -> Ferramentas (Think Tool)
create_arrow(ax, 4.5, 8.2, 2.5, 7, '2a. Usar', color_tool, offset=0.3)
create_arrow(ax, 2.5, 6.5, 4.5, 8.2, 'Resultado', color_tool, style='<->', offset=0.3)

# Agente -> Ferramentas (Write Todos)
create_arrow(ax, 4.3, 8, 2.5, 5.5, '', color_tool, offset=0.3)
create_arrow(ax, 2.5, 4.5, 4.3, 8, '', color_tool, style='<->', offset=0.3)

# Agente -> Ferramentas (Read Todos)
create_arrow(ax, 4.1, 7.8, 2.5, 3.5, '', color_tool, offset=0.3)
create_arrow(ax, 2.5, 2.5, 4.1, 7.8, '', color_tool, style='<->', offset=0.3)

# Agente -> Ferramentas (Web Search)
create_arrow(ax, 7.5, 8.2, 9.5, 7, '2b. Usar', color_tool, offset=0.3)
create_arrow(ax, 10.5, 6.5, 7.5, 8.2, 'Resultado', color_tool, style='<->', offset=0.3)

# Agente -> Ferramentas (Write Code)
create_arrow(ax, 7.7, 8, 9.5, 5.5, '', color_tool, offset=0.3)
create_arrow(ax, 10.5, 4.5, 7.7, 8, '', color_tool, style='<->', offset=0.3)
...
ax.text(6, 11.3, 'O agente decide dinamicamente qual ferramenta usar baseado no contexto', 
...
FERRAMENTAS:
• think_tool: Reflexao estrategica (recomendado apos web_search)
• write_todos: Criar/atualizar lista de tarefas (max 3 chamadas)
• read_todos: Ler lista atual de tarefas (max 5 chamadas)
• web_search: Buscar informacoes na web (max 3 chamadas)
• write_code: Gerar codigo usando grafo interno (max 3 chamadas)

O agente pode usar multiplas ferramentas em sequencia
ate ter informacao suficiente para responder."""

ax.text(0.3, 4, notes, ha='left', va='top', fontsize=9,
